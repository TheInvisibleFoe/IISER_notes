...
We shall be looking at how this equation of state is derived from a microscopic model as well as a mean-field treatment of the system. Then we shall take a look at where this theory succeeds and where it fails. Before we proceed, let us first understand the nature of molecular interactions.


== Molecular Interactions
To understand the behaviour of the real gas, we need to see how particles interact with each other. The most extensively studied potential of the pair-type potenials is
...
$
where $p_i$ is the momentum of the $i^(t h)$ particle, $m$ is the mass of the particles and $U(r_1, r_2, ..., r_N)$ is the total potential energy of the system. For our case we shal uuse a pairwise potential, which means that the total potential energy can be written as,
$
...
$
where $lambda = h/sqrt(2 pi m k_B T)$ is the thermal de Broglie wavelength. Now, the integral over the positions is difficult to perform due to the presence of the potential energy term. Firstly, we can rewrite the potential energy term as an integral over the inter-particle distances, rather than a summation. To do this, define the density function as,
$
