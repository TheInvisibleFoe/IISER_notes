...
Monumental efforts have been invested in the solution of the Ising Model in 3 Dimensions without much progress. The model can be extended to a mean field model that can be solved analytically. There are two ways to go about it, using the approach that Baxter uses and another using an inequality called the Bogoliubov inequality and a variational method @sakthivadivel_magnetisation_2022. We will outline the solution by Baxter. This model is pretty nice because we can graphically see why the phase transition takes place.
...
#figure(
  image("../codes/images/phi_dr.png", width: 100%),
...
#figure(
  image("../codes/images/mT1.png", width: 60%),
...
Here are some supplemental graphs to cement the fact that the peak scales with $N$ and the width of the peak scales as $N^(1/2)$, for both $T<T_c$ and $T>T_c$. These graphs are not really important, but since I spent some time coding and beautifying the graph, it would be a waste not to put it in the article.
#figure(
  image("../codes/images/crfordiffN.png", width: 80%),
  caption: [
...
#figure(
  image("../codes/images/crfordiffN2.png", width: 80%),
  caption: [
    The graph of $c_r$ vs $r$ for $T>T_c$ for different values of $N = 100, 200, 400, 800$
