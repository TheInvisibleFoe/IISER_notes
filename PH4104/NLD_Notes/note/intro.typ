...
#figure(
  // A quiver plot showing the flow field for the equation dx/dt = 1 + x^2
  // with arrows indicating the slope at various points.
  // (This is a placeholder; actual implementation would require plotting code.)
  image("images_1d/onedim_phaseplot_ex.png", width: 60%),
  caption: [Vector field (quiver plot) for $(d x) / (d t) = 1 + x^2$. The red line represents the trajectory starting from $x(0) = 0$.],
)
...
Generally to study bifurcations, we use the concept of normal forms, which are simplified equations that capture the essential behavior near the bifurcation point.
#definition("Normal Form")[
  Normal forms are simplified equations that capture the essential behavior of a system near a bifurcation point. All systems exhibiting the same type of bifurcation are topologically equivalent to the normal form of the bifurcation.
]
...
#subpar.grid(
  figure(
    image("images_1d/oned_phase_saddle_bef.png"),
    caption: [
  ...
  <a>,
  figure(
    image("images_1d/oned_phase_saddle.png"),
    caption: [
  ...
  <b>,
  columns: (1fr, 1fr),
  caption: [Before the bifurcation (a) we do not have any fixed points. After the bifurcation (b) two fixed points are created: one stable $mu$ and one unstable $-mu$.],
  label: <full>,
)
...
#figure(
  image(
    "images_1d/oned_bifurcation_saddle.png",
    width: 30%,
  ),
  caption: [The saddle node bifurcation occurs at $mu = 0$],
)
...
#subpar.grid(
  figure(
    image("images_1d/oned_phase_tc_bef.png"),
    caption: [
      Before the bifurcation.
    ],
  ),
  <a>,
  figure(
    image("images_1d/oned_phase_tc_aft.png"),
    caption: [
      After the bifurcation.
    ],
  ),
  <b>,
  columns: (1fr, 1fr),
  caption: [Before the bifurcation (a) there is one stable fixed point at x = $mu$ and one unstable $x=0$. After the bifurcation (b) there is one stable fixed point at $x = 0$ and one unstable $x=mu$. #text(red)[In the first figure the colors of the unstable and stable fixed points are swapped.]],
  label: <full>,
)
...
#figure(
  image(
    "images_1d/oned_bifurcation_tc.png",
    width: 30%,
  ),
  caption: [There is an exchange of stability at the point of the bifurcation.],
)
...
*Normal Form:* $ dot(x) = mu x - x^3 $

Fixed points:

Fixed points occur where $f(x) = 0$:
$ x_(plus.minus) = plus.minus sqrt(mu),0 $

- If *$mu < 0$:* One stable fixed point at $x=0$.
- If *$mu > 0$:* Two fixed points created, $x = +sqrt(mu)$ (stable) and $x = -sqrt(mu)$ (stable), with $x=0$ becoming unstable.


#subpar.grid(
  figure(
    image("images_1d/oned_phase_pitchfork_bef.png"),
    caption: [
      Before the bifurcation.
    ],
  ),
  <a>,
  figure(
    image("images_1d/oned_phase_pitchfork_aft.png"),
    caption: [
      After the bifurcation.
    ],
  ),
  <b>,
  columns: (1fr, 1fr),
  caption: [Before the bifurcation (a) there is only one stable fixed point at x = $0$. After (b) there is one unstable fixed point at $x = 0$ and two unstable $x=pm mu$. ],
  label: <full>,
)
One stable fixed point becomes unstable, and two new stable fixed points emerge symmetrically.
There are concepts of supercritical and subcritical bifurcations, which are discussed further in @strogatz2015
#figure(
  image(
    "images_1d/oned_bifurcation_pitchfork.png",
    width: 25%,
  ),
  caption: [Stable fixed point becomes unstable and two new stable ones emerge.],
)
#definition("Pitchfork Bifurcation")[

  A pitchfork bifurcation occurs when a single stable fixed point becomes unstable and gives rise to two new stable fixed points as a parameter is varied.
]
== Logistic Model (Population Growth)
Let $N$ be the population and $K$ be the carrying capacity and $r>0$ be the rate of growth.
$ (d N) / (d t) = r N (1 - N / K) $

Population does not grow indefinitely; it saturates. To simplify, we let $x = N/K$:
$ (d x) / (d t) = r x (1 - x) $
...
#figure(
  image(
    "images_1d/oned_phase_logistic.png",
    width: 50%,
  ),
  caption: [The phase plot of the logistic model. One unstable fixed point at zero population and a stable fixed point at the carrying capacity.],
)
